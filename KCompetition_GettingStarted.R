...
train = read.csv("train2016Copy.csv")

test = read.csv("test2016Copy.csv")
...
CARTMod = rpart(Party ~ . -USER_ID, data=train, method="class")

prp(CARTMod)

# Evaluate the performance of the model
predictCART = predict(CARTMod, newdata=test, type="class")
...
# Make predictions:
predictRF = predict(RFMod, newdata=test)

table(predictRF)

MySubmission = data.frame(USER_ID = test$USER_ID, Predictions = predictRF)
